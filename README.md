# C-Care — VDT 증후군 예방 모니터 거리 모니터링

> 웹캠 영상으로 사용자와 모니터 사이 거리를 분류해, 너무 가까워지면 자세 교정을 유도하는 데스크톱 사용 보조 도구입니다. 졸업 프로젝트.

오래 앉아 컴퓨터를 쓰면 무의식적으로 모니터에 가까워지면서 거북목·눈 피로 같은 VDT 증후군이 생깁니다. C-Care는 웹캠 프레임을 EfficientNetB0 분류기로 분석해 모니터와의 거리를 세 단계로 나누고, 화면에 색상·메시지로 피드백을 줍니다.
