...
모니터에 너무 가까이 붙어 오래 보면 눈이 쉽게 피로해집니다. 이 프로젝트는 웹캠 프레임을 EfficientNetB0(CNN) 분류기로 분석해 모니터와의 거리를 세 단계로 나누고, 너무 가까우면 화면에 색상·메시지로 경고해 시력 보호를 돕습니다.

## 역할과 범위

- **원본**: 팀 졸업 프로젝트(팀 "루모스")입니다. 원본에서 제가 맡은 부분은 **학습용 이미지 데이터 수집**(팀원·지인 대상 촬영), **경고 화면 프론트엔드**, **거리 분류 CNN 튜닝**입니다.
- **이 저장소**: 원본을 **개인적으로 재구현한 버전**입니다. 노트북 위주였던 원본을 서비스 형태로 다시 만들면서 **FastAPI 백엔드와 전처리·학습·예측 모듈 분리를 직접 추가**했습니다.
- 팀 전체 산출물과 상세 내용은 `assets/`의 보고서를 참고하세요.
...
- 원본은 팀 졸업 프로젝트이며, 제 담당 범위는 위 **역할과 범위**를 참고하세요.
