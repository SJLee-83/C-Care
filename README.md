# CNN을 활용한 시력 보호 모델 설계

웹캠 프레임을 EfficientNetB0 분류기로 분석해 모니터와 사용자 사이 거리를 세 단계로 나누고, 가까운 상태에서 색상·메시지로 경고하는 모델. 창의적 종합 설계(졸업 종합설계) 프로젝트 「CNN을 활용한 시력 보호 모델 설계」의 개인 재구현본이며, 저장소 이름 `C-Care` 는 편의상 붙인 것이다.
